...
export default function DocumentManager({ mode }: { mode: Mode }) {
  ...
  const load = async () => {
    try {
      const [optionsResult, documentResult] = await Promise.allSettled([request(`${base}/form-options`), request(base)]);
      if (optionsResult.status === "fulfilled") {
        setParties((isSales ? optionsResult.value.customers : optionsResult.value.suppliers) ?? []);
        setItems(optionsResult.value.items ?? []);
      }
      if (documentResult.status === "fulfilled") setDocuments(documentResult.value);
      const failed = [optionsResult, documentResult].find((result) => result.status === "rejected");
  ...
  const chooseItem = (index: number, itemId: string) => {
    setLines(lines.map((l, i) => i === index ? { ...l, itemId } : l));
  };
          ...
          <form className="form-grid" onSubmit={submit}>
            ...
            {lines.map((line, i) => (
              <div className="line-item" key={i}>
                ...
                <label className="field">{isSales ? "Unit price" : "Unit cost"}<input type="number" min="0" step="0.01" value={line.price} onChange={(e) => setLines(lines.map((x, n) => n === i ? { ...x, price: e.target.value } : x))} /></label>
